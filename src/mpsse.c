...
struct mpsse_context *MPSSE(enum modes mode, int freq, int endianess) {
    ...
    for (i = 0; supported_devices[i].vid != 0; i++) {
        if ((mpsse = Open(supported_devices[i].vid, supported_devices[i].pid,
                          mode, freq, endianess, IFACE_A, NULL, NULL)) !=
            NULL) {
            if (mpsse->open) {
                mpsse->description = supported_devices[i].description;
                break;
            }
            /* If there is another device still left to try, free the context
...
struct mpsse_context *OpenIndex(int vid, int pid, enum modes mode, int freq,
    ...
    if (mpsse) {
        ...
        if (ftdi_init(&mpsse->ftdi) == 0) {
            ...
            if (ftdi_usb_open_desc_index(&mpsse->ftdi, vid, pid, description,
                                         serial, index) == 0) {
                mpsse->mode = mode;
                mpsse->vid = vid;
                mpsse->pid = pid;
                ...
                status |=
                    ftdi_write_data_set_chunksize(&mpsse->ftdi, CHUNK_SIZE);
                status |=
                    ftdi_read_data_set_chunksize(&mpsse->ftdi, CHUNK_SIZE);
...
#error Only mac os or linux platforms supported
                                 ...
                                 * that SetMode may have sent. This clears out
                                 * any errors from unsupported commands that
                        ...
                        /* Skip the setup functions if we're just operating in
                         * BITBANG mode
                         */
                        if (ftdi_set_bitmode(&mpsse->ftdi, 0xFF,
                                             BITMODE_BITBANG) == 0) {
                            mpsse->open = 1;
                        }
                    }
...
int SetMode(struct mpsse_context *mpsse, int endianess) {
    ...
    if (mpsse) {
        ...
        case SPI0:
            ...
            mpsse->pstart &= ~SK;
            mpsse->pstop &= ~SK;
            /* SPI mode 0 propogates data on the falling edge and read data on
        ...
        case SPI3:
            ...
            mpsse->pstop &= ~SK;
            /* SPI mode 3 propogates data on the falling edge and read data on
             * the rising edge of the clock */
            mpsse->tx |= MPSSE_WRITE_NEG;
            mpsse->rx &= ~MPSSE_READ_NEG;
            mpsse->txrx |= MPSSE_WRITE_NEG;
        ...
        case SPI1:
            ...
            mpsse->pidle &= ~SK;
            /* Since this mode idles low, the start condition should ensure that
             * the clock is low */
            mpsse->pstart &= ~SK;
            /* Even though we idle low in this mode, we need to keep the clock
             * line high when we set the CS pin high to prevent an unintended
             * clock cycle from being sent by the FT2232. This way, the clock
             * goes high, but does not go low until after the CS pin goes high.
             */
            mpsse->pstop |= SK;
        ...
        case SPI2:
            ...
            break;
        case I2C:
            /* I2C propogates data on the falling clock edge and reads data on
             * the falling (or rising) clock edge */
            mpsse->tx |= MPSSE_WRITE_NEG;
            mpsse->rx &= ~MPSSE_READ_NEG;
            /* In I2C, both the clock and the data lines idle high */
            mpsse->pidle |= DO | DI;
            /* I2C start bit == data line goes from high to low while clock line
             * is high */
            mpsse->pstart &= ~DO & ~DI;
            /* I2C stop bit == data line goes from low to high while clock line
             * is high - set data line low here, so the transition to the idle
             * state triggers the stop condition. */
            mpsse->pstop &= ~DO & ~DI;
            /* Enable three phase clock to ensure that I2C data is available on
    ...
    if (is_valid_context(mpsse)) {
        if (mpsse->mode) {
            while (n < size) {
                 ...
                 * read back each individual ACK bit, so set the transmit size
                 * to 1.
                 */
                if (mpsse->mode == I2C) {
                    txsize = 1;
        ...
        if (mpsse->mode) {
            ...
            if (buf) {
                ...
                while (n < size) {
                    ...
                    data = build_block_buffer(mpsse, mpsse->rx, sbuf, rxsize,
                                              &data_size);
                    if (data) {
...
char ReadBits(struct mpsse_context *mpsse, int size) {
    ...
    if (rdata) {
        ...
        if (mpsse->endianess == MSB) {
            /*
             * In MSB mode, bits are sifted in from the left. If less than 8
             * bits were read, we need to shift them left accordingly.
             */
            bits = bits << (8 - size);
        } else if (mpsse->endianess == LSB) {
            /*
             * In LSB mode, bits are shifted in from the right. If less than 8
             * bits were read, we need to shift them right accordingly.
             */
            bits = bits >> (8 - size);
        }
 ...
 * Returns void.
 */
void SendAcks(struct mpsse_context *mpsse) { return SetAck(mpsse, ACK); }
 ...
 * Returns void.
 */
void SendNacks(struct mpsse_context *mpsse) { return SetAck(mpsse, NACK); }
...
int SetDirection(struct mpsse_context *mpsse, uint8_t direction) {
    ...
    if (is_valid_context(mpsse)) {
        if (mpsse->mode == BITBANG) {
            if (ftdi_set_bitmode(&mpsse->ftdi, direction, BITMODE_BITBANG) ==
                0) {
                retval = MPSSE_OK;
            }
        }
...
int ReadPins(struct mpsse_context *mpsse) {
    ...
    if (is_valid_context(mpsse)) {
        ftdi_read_pins((struct ftdi_context *)&mpsse->ftdi,
                       (unsigned char *)&val);
    }
...
int PinState(struct mpsse_context *mpsse, int pin, int state) {
    ...
    /* If not in bitbang mode, the specified pin should be one of GPIOLx.
     * Convert these defines into an absolute pin number. */
    if (mpsse->mode != BITBANG) {
        pin += NUM_GPIOL_PINS;
    }
